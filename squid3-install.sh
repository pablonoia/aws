...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 16.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/pablonoia/aws/master/squid.conf?token=ALFMIHVKKGXPNBC2RJLMXT26UQA2O
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/pablonoia/aws/master/squid.conf?token=ALFMIHVKKGXPNBC2RJLMXT26UQA2O
    /sbin/iptables -I INPUT -p tcp --dport 3128 -j ACCEPT
    /sbin/iptables-save
